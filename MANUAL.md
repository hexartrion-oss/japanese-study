...
- **학습 항목 반복 회피는 N1/N0 문법·관용표현에만 적용된다.** `study_history.json`
  최근 20회 윈도우로 회피 지시하는 대상은 `_gemini_study_focus`가 다루는
  문법·관용표현뿐이며, 본문 어휘 전반이나 장르·관점 시드(`_pick_adv_seed`)의
  반복은 대상이 아니다. 또한 Gemini가 회피 지시를 무시할 수도 있어
  100% 반복 차단을 보장하지 않는다(프롬프트 지시 수준의 회피).
