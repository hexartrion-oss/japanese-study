...
LEVEL_DESC = {
    ...
    "JLPT N0": {
        ...
        "example": (
            "量子コンピュータの実用化は、現行の公開鍵暗号体系に根本的な再考を迫るものと広く予見されている。"
            "ショアのアルゴリズムが示すように、十分な量子ビットを有するシステムは素因数分解を多項式時間で解くことが可能となり、"
            "現在のRSA暗号が前提とする計算量的安全性は原理的に崩壊しうる。"
            "この脅威に対応すべく、格子暗号をはじめとする耐量子暗号の標準化が国際的に進められているところである。"
            "技術革新と安全保障の相克は、今後の情報社会における中心的な政策課題として浮上するであろう。"
        ),
    },
}
