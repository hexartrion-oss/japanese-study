...
MANUAL_MAIL_TO = MANUAL_MAIL_TO_ENV or GMAIL_ADDRESS  # 수동 실행 시 유일한 수신자 (secrets로 지정)
...
# 경어 모드: 문서 유형 (사내/사외 구분 — 자사 상사·동료를 사외 인물과
# 동일하게 존경어로 다뤄버리는 오류를 막기 위해 목록을 분리한다)
_SEED_KEIGO_DOC_INTERNAL = [
    "上司への社内報告メール",
    "会議の議事録",
    "業務の引き継ぎ文書",
]
_SEED_KEIGO_DOC_EXTERNAL = [
    "取引先への依頼・案内文",
    "クレームへの対応・お詫び文",
    "社外向けプレゼンテーションの原稿",
]
...
def write_story_with_gemini(theme: str, label: str, attempt: int = 0,
    ...
    ab_stance = ""
    seed_lines = ""
    doc_kind = None  # 경어 비즈니스 문서 전용: "internal"(사내향) / "external"(사외향)
    if is_beginner:
        ...
        if business_doc:
            # 사내향/사외향을 먼저 정해야 문체 지시(존경어 vs 겸양어 대상)를 올바르게 분기할 수 있다.
            doc_kind = random.choice(["internal", "external"])
            _sd = random.choice(
            ...
            _rlog(f"[시드] 경어 문서: {_sd} ({doc_kind}) / 포커스: {_sf[0][:20]}... + {_sf[1][:20]}...")
        else:
...
・一続きの体験談として自然に流れる文章にすること"""
        ...
        if doc_kind == "internal":
            # 사내향 문서: 읽는 사람이 사내 인물(상사 포함)이거나, 사내 인물의 행동을
            # 다른 사내 인물에게 전달하는 문서 → 자사 인물에게는 존경어를 쓰지 않는다 (우치소토 원칙)
            style_instruction = f"""【文体・敬語ルール（社内向け文書）】
・謙譲語（「〜いたします」「拝見する」「お伺いする」等）・丁重語・丁寧語（「〜でございます」「〜ております」等）を中心に使うこと
・自社の人物（上司・部長・同僚など）の行為を第三者（読み手を含む社内の人）に説明するときは、
  尊敬語ではなく謙譲語・丁重語・普通の表現を使うこと
  （例：「ご対応なさる」ではなく「対応する／対応いただく」、「いらっしゃいます」ではなく「います／おります」、
  「ご帰社次第」ではなく「帰社次第／戻り次第」）
・尊敬語は、社内の目上の人物に直接語りかけ、その人物自身に依頼する場面（「〜いただけますでしょうか」等の依頼表現）に限って最小限使ってよい。
  上司の行動を単に説明・報告する文では尊敬語を使わない
・ビジネスメール・報告書・議事録・引き継ぎ文書など実際の実務場面で使われる表現を中心にすること
・文末表現の偏り禁止：「〜いたします」「〜ております」「〜させていただきます」など、同じ文末表現は文書全体で2回まで。同じ文末を2文連続で使わない
{_NO_FILLER_RULE}
・感情描写や登場人物の心理描写は禁止"""
        else:
            style_instruction = f"""【文体・敬語ルール（社外向け文書）】
...
・謙譲語だけに偏らず、読み手・相手（社外の取引先・顧客）の行為への尊敬語（ご覧になる・おっしゃる・お越しになる・ご確認なさる等）も3文以上で使うこと
{_NO_FILLER_RULE}
・感情描写や登場人物の心理描写は禁止"""
...
def send_email(date_str: str, label: str, mode: str):
    ...
    try:
        if MANUAL_RUN:
            # 수동 실행: MANUAL_MAIL_TO(secrets) 단독 수신
